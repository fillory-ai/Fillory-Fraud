...
import type { AlertData, ConfigStatusData, PropertyData, ScanLogData, ScrapedListingData, StatsData } from "./types";

async function api<T>(url: string, init?: RequestInit): Promise<T> { const response = await fetch(url, init); if (!response.ok) throw new Error(await response.text()); return response.json() as Promise<T>; }
function App() {
  const [tab, setTab] = useState<TabId>("dashboard"); const [stats, setStats] = useState<StatsData | null>(null); const [config, setConfig] = useState<ConfigStatusData | null>(null); const [properties, setProperties] = useState<PropertyData[]>([]); const [listings, setListings] = useState<ScrapedListingData[]>([]); const [scans, setScans] = useState<ScanLogData[]>([]); const [alerts, setAlerts] = useState<AlertData[]>([]); const [fraudListings, setFraudListings] = useState<ScrapedListingData[]>([]); const [loading, setLoading] = useState(true); const [filter, setFilter] = useState("all"); const [source, setSource] = useState("all"); const [scanning, setScanning] = useState(false); const [dialogOpen, setDialogOpen] = useState(false); const [editing, setEditing] = useState<PropertyData | null>(null); const [saving, setSaving] = useState(false);
  const refresh = useCallback(async () => { setLoading(true); try { const [s, c, p, sh, a, f] = await Promise.all([api<StatsData>("/api/stats"), api<ConfigStatusData>("/api/config/status"), api<PropertyData[]>("/api/properties"), api<ScanLogData[]>("/api/scans"), api<AlertData[]>("/api/alerts"), api<ScrapedListingData[]>("/api/listings?fraud_status=fraud&limit=50")]); setStats(s); setConfig(c); setProperties(p); setScans(sh); setAlerts(a); setFraudListings(f); } finally { setLoading(false); } }, []);
  ...
  useEffect(() => { void refresh(); }, [refresh]); useEffect(() => { void loadListings(filter); }, [filter, loadListings]);
  ...
  const saveProperty = async (values: PropertyFormValues) => { setSaving(true); const payload = { ...values, zip_code: values.zip_code || null, bedrooms: values.bedrooms ?? null, bathrooms: values.bathrooms ?? null, square_footage: values.square_footage ?? null, monthly_rent: values.monthly_rent ?? null, description: values.description || null, amenities: values.amenities.split(",").map(v => v.trim()).filter(Boolean) }; try { await api(editing ? `/api/properties/${editing.id}` : "/api/properties", { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); setDialogOpen(false); await refresh(); } finally { setSaving(false); } };
  ...
  return <div className="min-h-screen bg-muted/30"><TopNav activeTab={tab} onTabChange={setTab} /><main className="mx-auto max-w-7xl px-4 py-8 md:px-6">{tab === "dashboard" && <DashboardTab stats={stats} config={config} listings={fraudListings} properties={properties} loading={loading} />}{tab === "import" && <ImportTab onImported={refreshAfterImport} />}{tab === "properties" && <PropertiesTab properties={properties} open={dialogOpen} editing={editing} saving={saving} onOpenChange={setDialogOpen} onAdd={() => { setEditing(null); setDialogOpen(true); }} onEdit={p => { setEditing(p); setDialogOpen(true); }} onDelete={deleteProperty} onSave={saveProperty} />}{tab === "listings" && <ListingsTab listings={listings} properties={properties} filter={filter} onFilterChange={setFilter} onDelete={deleteListing} loading={loading} />}{tab === "scans" && <ScansTab scans={scans} source={source} onSourceChange={setSource} onRunScan={runScan} scanning={scanning} />}{tab === "alerts" && <AlertsTab alerts={alerts} />}</main></div>;
}
export default App;
