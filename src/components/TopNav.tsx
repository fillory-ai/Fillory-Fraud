import { Activity, BarChart3, Bell, ClipboardPaste, FolderCheck, Home, ScanLine, Search, ShieldCheck } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export type TabId = "dashboard" | "import" | "properties" | "listings" | "cases" | "scans" | "alerts";
...
const tabs = [
  ...
  { id: "cases", label: "Cases", icon: FolderCheck },
  { id: "scans", label: "Scans", icon: ScanLine },
  { id: "alerts", label: "Alerts", icon: Bell },
] as const;

export function TopNav({ activeTab, onTabChange, observeMode, openCases }: TopNavProps) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          {observeMode && (
            <Badge variant="outline" className="gap-1 border-blue-500/20 bg-blue-500/10 text-blue-600">
              <ShieldCheck className="h-3 w-3" /> Observe mode
            </Badge>
          )}
        </div>
        <nav className="flex gap-1 overflow-x-auto" aria-label="Main navigation">
          {tabs.map(({ id, label, icon: Icon }) => (
            <Button key={id} variant="ghost" size="sm" onClick={() => onTabChange(id)} className={cn("shrink-0 gap-2 text-muted-foreground", activeTab === id && "bg-muted text-foreground")} aria-current={activeTab === id ? "page" : undefined}>
              ...
              {id === "cases" && openCases != null && openCases > 0 && (
                <span className="ml-0.5 rounded-full bg-amber-500/15 px-1.5 text-xs font-semibold text-amber-600">{openCases}</span>
              )}
            </Button>
          ))}
        </nav>
